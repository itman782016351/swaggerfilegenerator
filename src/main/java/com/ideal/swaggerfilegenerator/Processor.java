...
public class Processor {
    ...
    public Map getRenderMap(File f) throws IOException {
        ...
        for (int i = 0; i <=sheet.getLastRowNum(); i++) {
            ...
            reqEnd=respStart-1;
            //处理固定信息
            String cell1Val = row.getCell(1).getStringCellValue();
            String cell2Val = row.getCell(2).getStringCellValue();
            switch (cell1Val) {
                case "服务名": fixInfo.put("serverName",cell2Val);
                break;
                case "服务版本": fixInfo.put("serverVersion",cell2Val);
                    break;
                case "API名称": fixInfo.put("apiName",cell2Val);
                    break;
                case "API描述": fixInfo.put("apiDesc",cell2Val);
                    break;
                case "EOP基础路径": fixInfo.put("eopBasePath",cell2Val);
                    break;
                case "资源动作": fixInfo.put("httpMethod",cell2Val);
                    break;
                case "资源地址": fixInfo.put("resourcePath",cell2Val);
                    break;
                case "提供方Host（测试）": fixInfo.put("providerHost4Test",cell2Val);
                    break;
                case "提供方basePath（测试）": fixInfo.put("providerBasePath4Test",cell2Val);
                    break;
                 default:break;
            }
        }
        ...
        for (int i = 0; i < sheet.getLastRowNum(); i++) {
            ...
            String cell1Val = row.getCell(1).getStringCellValue();
            String cell2Val = row.getCell(2).getStringCellValue();
            String cell3Val = row.getCell(3).getStringCellValue();
            String cell4Val = row.getCell(4).getStringCellValue();
    ...
    public  boolean collectionJudge(String para){
        ...
        switch (para) {
            ...
            case "1-n": result = true;break;
            case "0-n": result = true;break;
            case "0-1": result = false;break;
            default: break;
        }
        return  result;
    }
